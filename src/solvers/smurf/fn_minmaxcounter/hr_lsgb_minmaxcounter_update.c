...
void CalculateMINMAXCOUNTERStateLSGBHeuristic(SmurfManager *SM, SmurfInfoStruct *pSmurfInfo) {
  ...
  if(nMax==1) {
    ...
    fPosValue = arrGetStateHeuristicScore[pMINMAXCOUNTERState->pVarIsTrueTransition->type](SM, pMINMAXCOUNTERState->pVarIsTrueTransition);
  }

  if(nNumVariables-1 == nMin) {
    fNegValue = ((double)(nNumVariables-1)) * SM->JHEURISTIC_K_INF;
  } else {
    fNegValue = arrGetStateHeuristicScore[pMINMAXCOUNTERState->pVarIsFalseTransition->type](SM, pMINMAXCOUNTERState->pVarIsFalseTransition);
  }
