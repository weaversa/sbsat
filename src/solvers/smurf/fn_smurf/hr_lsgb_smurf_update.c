...
void CalculateSmurfLSGBHeuristic(SmurfManager *SM, SmurfInfoStruct *pSmurfInfo) {
  ...
  SmurfStateTransition *pTransition = (SmurfStateTransition *)(pSmurfState + 1);
  for(uintmax_t i = 0; i < nNumVariables; i++) {
    if(pTransition->pVarIsFalseTransition == (TypeStateEntry *)pSmurfState) {
      pTransition++;
      continue; //Var is not in Smurf
    }
    
    SM->arrPosVarHeurWghts[pIndex2Var[i]] += pTransition->fHeurWghtOfTrueTransition;
    SM->arrNegVarHeurWghts[pIndex2Var[i]] += pTransition->fHeurWghtOfFalseTransition;
    pTransition++;
  }
}
