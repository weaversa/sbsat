...
uint8_t IsVariableAssigned_smurf(SmurfManager *SM, uintmax_t nVariable) {
  ...
  intmax_t decision_level = FS->level.data[nVariable];
  d9_printf3("Decision level of %ju is %jd\n", nVariable, decision_level);
  if(decision_level == -1) {
    return 0;
  } else {
    intmax_t infqueue_level = FS->model.data[nVariable];
    d9_printf4("Inference queue level of %ju is %jd (propq=%jd)\n", nVariable, infqueue_level, FS->propq);
    if(infqueue_level <= FS->propq) {
      return 1;
    } else {
      return 0;
    }
...
uint8_t SmurfSolve(SmurfManager *SM) {
  ...
  while (!FS->conf->isResourceLimitHit(FS, FS->conf->user)) {
	...
	trailPop(FS, NULL);
      ...
      trailPush(FS, -flipme, UINTPTR_MAX /*NO_REASON*/);

      continue;
    }

    /*
    if (!funcsatMakeDecision(FS, FS->conf->user)) {
...
SmurfManager *Init_SmurfSolver(BDDManager *BM, funcsat *FS) {
  ...
  funcsatResize(FS, BM->nNumVariables); //SEAN!!! eh? This is sort of important to get right for counting solutions.
