...
void CalculateORCOUNTERStateLSGBHeuristic(SmurfManager *SM, SmurfInfoStruct *pSmurfInfo) {
  ...
  uint8_t *barrPolarity = pORCOUNTERState->pORState->bPolarity;
  uintmax_t *pIndex2Var = pSmurfInfo->pIndex2Var;
  
  double fPosValue = SM->JHEURISTIC_K_TRUE;
  double fNegValue = arrGetStateHeuristicScore[pORCOUNTERState->pTransition->type](SM, pORCOUNTERState->pTransition);

  uintmax_t nNumFound = 0;

  for(uintmax_t i = 0; nNumFound < nNumVariables; i++) {
    if(barrPolarity[i] != 2  && SM->pTrail[pIndex2Var[i]]==0) {
      nNumFound++;
      SM->arrPosVarHeurWghts[pIndex2Var[i]] += barrPolarity[i]?fPosValue:fNegValue;
      SM->arrNegVarHeurWghts[pIndex2Var[i]] += barrPolarity[i]?fNegValue:fPosValue;
    }
  }
}
